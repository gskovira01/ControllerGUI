...
while True:
    ...
    if event in NUMERIC_KEYPAD_BUTTONS:
        ...
        continue
    # Poll actual position every second (on timeout)
    if event is None:
        # Determine active tab/servo
        active_tab = values.get('TABGROUP', 'TAB1')
        try:
        ...
        if comm:
            pos_resp = comm.send_command(pos_cmd)
            print(f'[POLL] Response: {pos_resp}')
            # Update GUI actual position field
            try:
                pos_val_pulses = float(pos_resp.strip().replace(':',''))
                scaling = AXIS_UNITS[axis_letter]['scaling']
                gearbox = AXIS_UNITS[axis_letter]['gearbox']
                pos_val_deg = pos_val_pulses / (scaling * gearbox)
                window[f'S{active_servo}_actual_pos'].update(str(round(pos_val_deg, 2)))
            except Exception:
                window[f'S{active_servo}_actual_pos'].update('N/A')
        continue
